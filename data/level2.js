...
quiz:[
{q:"SCENARIO: Windows install done, but 'python' command not found in terminal. Your setup is: installer finished without error. What's the critical step missed?",o:["Install to the C: drive root","Check 'Add Python to PATH' so the terminal can find Python","Disable antivirus"],a:1,e:"Mental model: PATH is the shell's search list for executables. Common mistake: skipping the checkbox during install. Check this question: without PATH, Python exists but is invisible to the terminal."}]},
...
quiz:[
{q:"You set max_tokens=30 on a customer support bot to keep answers short, but it now cuts off mid-sentence on every real reply. The prompt still says 'be concise'. Fix the broken length control.",o:["The model ignores prompts","max_tokens is a hard cutoff (guillotine), not a style request. It amputates at N tokens regardless of meaning. Ask for 'one-sentence answer' in the prompt; use max_tokens only as safety cap.","Raise temperature"],a:1,e:"Mental model: prompting controls style; max_tokens is a safety valve. Common mistake: using the safety knob for output shaping. Check this question: for controlled brevity, put the instruction in the prompt first; reserve max_tokens for runaway protection."},
...
quiz:[
{q:"DIAGNOSTIC: Your Twin CLI bot had a failing case on \"refund policy for 2024\". You tweaked the system prompt and it now passes that case. Your setup is: only that one test. What must you do before declaring victory?",o:["Ship it — the demo works","Re-run the entire eval suite on all test cases; prompt edits fix one path and commonly regress unrelated cases (e.g. tone or other topics) that you no longer check","Only measure latency"],a:1,e:"Mental model: prompts are global rewrites of behavior. Common mistake: local fixes without regression guard. Check this question: a real eval harness (built in this level) protects the whole Twin, not just the case you stared at."},
{q:"Your Twin at L2 state produces fluent, confident-sounding wrong answers on edge cases. Why is eyeballing the output screen insufficient for production?",o:["Models are usually correct by default","LLM mistakes wear the costume of correctness — polished prose, perfect grammar, authoritative tone hide factual or logical errors that only systematic test-set scoring catches","You just need higher temperature"],a:1,e:"Mental model: fluency camouflage is the #1 reason demos lie. Common mistake: trusting surface reading after a few happy runs. Check this question: the eval harness you build here forces you past vibes into numbers that survive real traffic."}]},
...
quiz:[
{q:"What is an agentic coding tool like Claude Code, in terms of what you've learned?",o:["A faster autocomplete","An agent — the reason/act/observe loop with tools — pointed at your codebase: it reads files, runs commands, and edits code to complete tasks","A new programming language"],a:1,e:"Mental model: the agent loop (reason, act via tools, observe) applied to code. Common mistake: treating it as magic autocomplete. Check this question: understanding the loop (which you have) is exactly why you can use (and trust, and debug) these tools well."},
{q:"While you're still building fundamentals, what's the right way to use AI coding tools?",o:["Let them write everything so you ship faster","Read every line and be able to explain it; use them to go faster on things you understand, not to skip understanding","Avoid them entirely until you're an expert"],a:1,e:"Accelerate, don't outsource. The 'read every line, explain every line' standard protects the skill-building. Once you can critique any diff, you can lean in much harder."}]},
...
quiz:[
{q:"You're about to implement function calling and structured output. What's the smart first move?",o:["Search for any blog post and copy it","Check the provider's official cookbook (OpenAI/Anthropic) for a working, current recipe before writing it from scratch","Guess the API and fix errors as they come"],a:1,e:"Cookbooks are curated, working, and current. Starting from a primary-source recipe beats reconstructing a pattern from a possibly-outdated blog — and you still read and understand it."},
{q:"Why prefer official docs over older tutorials?",o:["Docs are shorter","APIs and best practices change fast; the provider's own docs are the source of truth, while an old tutorial may now be wrong","Tutorials are never useful"],a:1,e:"The field moves monthly. Primary sources (docs, cookbooks, papers) stay current and authoritative; reposted blog knowledge drifts stale. Knowing where to look is a core skill."}]}
]}
],
