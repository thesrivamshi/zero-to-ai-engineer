...
quiz:[
{q:"PRODUCTION: Your support bot's answers on this week's new product catalog are outdated and wrong. The catalog changes every Monday. A teammate suggests 'just fine-tune weekly on the new docs'. Your setup has no current eval harness. What is the correct diagnosis and first move?",o:["Fine-tune weekly — it will learn the new facts","Knowledge problem: fresh facts belong in RAG (update the index nightly). Fine-tuning teaches style/behavior, not a database; without evals you can't even know if it helped. Build the eval + RAG update first.","Add the catalog to the system prompt"],a:1,e:"Mental model: RAG decouples knowledge from weights (facts stale → update store). Common mistake: using fine-tune for transient knowledge. Check this question: always measure baseline prompt/RAG before any fine-tune; the catalog case is the textbook misuse."},
{q:"SCENARIO: Your 50k-log fine-tune run after 3 weeks of LoRA gave +3% on internal metric vs the untuned frontier. No strong prompt baseline was measured. Production traffic will be 100k calls/day. What's the likely next reality per the lesson?",o:["Ship immediately — +3% is a win","You probably still need the skipped data work (curation/hard negatives) or the prompt baseline may have matched or beaten it; at volume the real question is whether the cheaper small model clears your full eval + cost bar","The gain will compound"],a:1,e:"Mental model: data >> training knobs; baseline first. Common mistake: declaring victory after one training run without the comparison. Check this question: before shipping fine-tune, show delta vs best prompt-only on held-out, and cost/latency at your scale."}]},
...
quiz:[
{q:"Your team spent 3 weeks on LoRA training runs and got a 3% lift on the internal eval. The dataset was 50k raw logs with minimal filtering. What does the lesson predict about next steps?",o:["Ship it — 3% is good","The real work was skipped: curation, hard-example mining, and rigorous eval against a strong prompt baseline. Most effort belongs in the data, not the training job; you likely still need to do the data work to see real gains or decide fine-tuning wasn't needed","Run longer"],a:1,e:"Mental model: training knobs are easy; the model is the data. Common mistake: declaring victory after the first training run. Check this question: before declaring fine-tuning success, show the delta vs your best prompt-only baseline on a held-out set."},
...
quiz:[
{q:"FIX THIS DATA FOR YOUR TWIN: You have 50k raw historical support logs. Plan: dump them into fine-tune for voice Twin. What is the correct first step per dataset lesson?",o:["Train on all of them — more data is better","Curate ruthlessly: keep only excellent consistent responses, enforce style guide, deliberately include edge/hostile cases (even if only ~2k survive) — the model becomes exactly its training outputs","Use raw logs and hope the model averages well"],a:1,e:"Mental model: fine-tune data (L4b1) is the product. Common mistake: quantity over quality. Check this question: bad historical replies get baked into permanent Twin behavior; curate like you would brand guidelines."},
{q:"SCENARIO: Your Twin voice fine-tune data contains only clean polite exchanges. In prod a user is hostile + asks for private info leak. Model complies or hallucinates. Root cause per this lesson?",o:["Model is misaligned by default","Hostile/ambiguous/edge cases were absent from data, so no learned correct response (e.g. refusal + privacy guard) was installed; dataset designs the behavior distribution","Bad system prompt only"],a:1,e:"Mental model: the dataset (not prompt) teaches desired responses to real inputs. Common mistake: train on happy path only. Check this question: diversity and coverage in L4 data directly determine robustness of your voice Twin later used in L5 multi-agent."}]},
