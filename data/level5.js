...
quiz:[
{q:"At 3am your FTI feature pipeline (the nightly scraper+embedder) crashes hard. Users are asking questions right now. What is the immediate user-visible effect, and why?",o:["The whole product is down","Users get yesterday's index (stale but correct-enough answers) because inference reads the vector store independently; the broken feature pipeline does not take down serving.","All answers turn into hallucinations"],a:1,e:"Mental model: FTI decouples update from serve. Common mistake: treating the whole RAG app as one script. Check this question: a failed indexer leaves inference serving the last good snapshot — exactly the resilience you design for."},
...
videos:[{title:"FastAPI — official tutorial (User Guide)",url:"https://fastapi.tiangolo.com/tutorial/",why:"The canonical step-by-step guide. Work through it once and you'll understand the framework powering your project's API."}]},
...
videos:[{title:"TechWorld with Nana — Docker Tutorial for Beginners [Full Course, 3h]",url:"https://www.youtube.com/watch?v=3c-iBn73dDE",why:"The clearest end-to-end Docker course out there. Watch the first hour before the deploy lab; it makes containers click."}]},
...
{q:"Your CFO asks why last month's RAG feature bill is 4× higher than the '5¢ per answer' you quoted. The dashboard shows only the final LLM call. What's the missing diagnosis?",o:["The CFO is wrong","Auxiliary calls (embeddings for every query + rewrite + judge + guardrail) are not in the headline number; each user question triggers a bundle. Trace per-stage cost or you will always underprice.","Inflation"],a:1,e:"Mental model: the visible call is rarely the only call. Common mistake: quoting only the 'main' generation price. Check this question: instrument every auxiliary LLM/embed call; the real unit economics are the sum."}]},
...
{id:"l5quickmcp",t:"⚡ 5-min quick win: feel MCP list/call by hand + share your Twin MCP progress",min:5,body:`
<p>Before the full lab: run this 8-line snippet. It is the entire soul of MCP (tools/list + tools/call discovery + dispatch — and your future Twin MCP server for the explicit culmination at L5 as full MCP-exposed multi-agent system: supervisor + voice writer + retriever + critic).</p>
<pre><code>TOOLS = {"add": lambda a,b: a+b, "recall": lambda k: "memory:"+k}
def handle(m,p):
  if m=="tools/list": return [{"name":n, "description":f"tool {n}"} for n in TOOLS]  # full discovery
  if m=="tools/call": 
    if not validate_args(p): return {"error":"consent+validate required"}  # security
    return {"result": TOOLS[p["name"]](**p["arguments"])}
print(handle("tools/list",{}))  # client discovers
print(handle("tools/call",{"name":"add","arguments":{"a":2,"b":3}}))  # secure call</code></pre>
<p>Change it. Add your Twin tools (ask_my_twin). Add a fake validate + consent stub for security best practice. This mechanic + JSON-RPC + stdio/StreamableHTTP + resources/prompts is what the real SDK + gateways wrap — now you own the production shape. Instant + share.</p>`,
quiz:[{q:"FIX THIS BROKEN PROD SETUP: Your Twin tools (recall, search_docs) work only inside your script. A teammate's Claude Desktop and a new internal dashboard both need the exact same two tomorrow. Current state: separate glue per client. What minimal MCP mechanic solves it?",o:["Add descriptions to every client","Expose via MCP server implementing tools/list + tools/call; any MCP client discovers and invokes the same shared implementation","Duplicate the SQL + retrieval code everywhere"],a:1,e:"Mental model: MCP (introduced in this L5 d1) turns private functions into a discoverable server. Common mistake: per-client wiring forever. Check this question: list advertises, call dispatches — the exact mechanic you build in the lab and use for Stage 3 Twin."}]},
...
<p>You build one <strong>MCP server</strong> that exposes capabilities; every MCP-aware client (Claude, Cursor, your own agent, OpenAI Agents SDK) speaks the same protocol to it. A server offers three things:</p>
<ul>
...
<p>Under the hood it's [[json-rpc|JSON-RPC 2.0]] (exact envelope: {"jsonrpc":"2.0", "id":1, "method":"tools/list", "params":{}} → result with structured content). Core primitives (full MCP spec): <strong>Tools</strong> (executable actions with inputSchema, list + call), <strong>Resources</strong> (read-only context: list resources + read resource URI, subscribe for updates), <strong>Prompts</strong> (server-provided templates: list prompts + get prompt with args). Client capabilities: Sampling (ask server to generate), Roots (workspace roots), Elicitation (structured user input). Transports: <strong>stdio</strong> (local subprocess, stateful) or <strong>Streamable HTTP</strong> (remote, stateless core + SSE for streaming notifications/responses; supports 2026 RC stateless scaling on ordinary HTTP infra). Full security best practices (MUST): explicit user consent & control before any data/execution; treat all tool/resource/prompt descriptions as untrusted (injection vector); least-privilege scoping + arg validation always; auth (OAuth 2.1 for remote); no auto-execution of side effects; PII redaction before remote; user approval for sampling results. MCP is an open standard created by Anthropic, with a fast-growing ecosystem of clients and servers. For production, MCP gateways (e.g. Tyk, Kong, Cloudflare) add OAuth, per-token/cost rate limits, tool filtering, and unified tracing/observability.</p>
<p><strong>2026 efficiency pattern (code execution with MCP, Anthropic + Cloudflare 2025/26)</strong>: Direct tool calls bloat context with every def + every intermediate result (e.g. full transcripts/docs duplicated — 50-100k tokens common). Instead: expose MCP servers as code modules in a FS tree (e.g. ./servers/google-drive/getDocument.ts or Python equiv). Agent discovers on-demand (list dir / read file = progressive disclosure), writes/executes code in sandboxed env (secure exec with limits) to compose calls, filters/transforms data (e.g. 10k rows → 5 pending summaries) BEFORE model sees it. Example: 150k tokens → 2k (98.7% cut); parallel for multiple MCPs. Keeps large intermediates private (only logs/returns what you choose). Privacy: harness tokenizes PII (e.g. email→ [EMAIL_1]), untokenizes on real calls. State/skills: write reusable code + data to FS for resume across runs; add SKILL.md for progressive capabilities. Handles 1000s of tools scalably without context explosion. MCP gateways (2026) layer on top: OAuth 2.1, per-token/cost rate limits, tool filtering/progressive disclosure enforcement, unified observability — essential when your Twin/MCP tools are called by supervisor multi-agents at scale. Composio example: managed MCP-like tool server for 100+ SaaS (Gmail, Slack, etc) — register once, your Twin supervisor consumes via list/call without custom auth code per integration.</p>
<div class="callout"><div class="ct">Why this is the agent spine's payoff</div>Your agent already has <code>remember</code>/<code>recall</code> (SQL memory) and other tools. Wrap them in an MCP server and they stop being private functions — they become capabilities any client can plug into. That's how a personal agent becomes infrastructure other tools build on. (FastMCP Python: @mcp.tool() decorators make it trivial.) 2026 reality: MCP gateways (Tyk/Kong/Cloudflare/TrueFoundry etc) sit in front for auth (OAuth 2.1), token/cost rate limiting, progressive disclosure enforcement, and unified observability — essential when your Twin/MCP tools are called by supervisor multi-agents at scale. (Fresh 2026: 10k+ public MCP servers; MCP is table stakes + LangGraph Postgres checkpointers enable durable resume/time-travel for production supervisor Twins.)</div>`,
quiz:[
{q:"DIAGNOSE & FIX: Your internal Twin agent has working recall wired only to its script. Design team wants Claude Desktop + web app to use identical memory tool. State: per-client function calling glue. Minimal change?",o:["Add more function descriptions in each client","Publish remember/recall as MCP server exposing tools/list+call; any MCP client discovers and calls the same impl","Re-implement the SQL queries in every client"],a:1,e:"Mental model: MCP (L5) standardizes discovery+dispatch once. Common mistake: duplicating glue per consumer. Check this question: server owns capability; your Stage 3 Twin lab turns this into the exposed multi-agent core."},
{q:"Your production MCP server for Twin tools is live over stdio. A remote dashboard wants to use the same 'ask_twin' tool. Your setup is in state: stdio only + no extra guards. What transport + discipline upgrade?",o:["Nothing — stdio works everywhere","Switch to HTTP transport for remote; apply least-privilege, auth, arg validation, and approval gates on dangerous tools because a remote MCP server is an API the model can drive via injection","Use bigger models"],a:1,e:"Mental model: stdio is local subprocess; HTTP is the network case. Common mistake: 'MCP makes it automatically safe'. Check this question: remote = production API shape, so all injection/guardrail lessons from agents apply at full strength."},
{q:"PRODUCTION SCALE SCENARIO: In Anthropic's code-execution-with-MCP pattern, how does presenting tools as code files + sandbox cut tokens 98.7%, and what 2026 infra helps scale it for your Twin?",o:["It duplicates data into more calls","Progressive disclosure (agent reads only needed tool files), filtering/transforms happen in exec env (model sees tiny output), intermediates stay private; MCP gateways add token/cost rate limits + observability for supervisor multi-agent at scale","It requires bigger context windows"],a:1,e:"Mental model: direct defs + full results bloat context. Code mode + gateways enforce progressive disclosure + cost control. Common mistake: exposing full tool defs+results to model. Check this question: gateways (2026) are table stakes for multi-MCP production." },
{q:"Per latest MCP spec security principles (2025-11-25) added to the lesson, what MUST be done for tool use on a remote MCP server?",o:["Let the model decide freely","Obtain explicit user consent before invoking (tools are arbitrary execution); treat tool descriptions as untrusted; validate args + least-privilege + gates always","Skip if using stdio"],a:1,e:"Spec mandates consent/control, privacy, safety. Model-driven remote tools = production API + injection risk — consent + validation contain it." }]},
...
<p><strong>ADHD micro + community productization:</strong> Note your 2+ tools (e.g. ask_my_twin + supervisor_handoff). Complete this → immediately use floating 🚀 Share your Twin or ⚡Share+Discord (live MD from P/AGENT/gates will include 🔌MCP + your tools). Builds streak + locks progress for cohort. Choice: which MCP tool first?</p>`,
py:{
...
{q:"After this 5-min MCP micro, what should you do per the course productization?",o:["Ignore it","Complete + hit the prominent floating 🚀 Share Twin (or ⚡Share+Discord) to paste live snippet with MCP badge/tools to Discord for accountability (uses P/AGENT/gates state)","Skip sharing"],a:1,e:"All quick wins tie to share flows + streaks for dopamine + community. Your Twin culminates as full MCP multi-agent — share it!"} ,
...
{id:"l5d2quick",t:"⚡ Micro: secure MCP list+call + gateway exposure for Twin",min:4,src:"AIE ch.6 §Tools (MCP)",body:`<p>Extend your L5 MCP lab for Twin production: after basic list/call, 1) add arg validator + consent stub (never auto-execute side effects). 2) Simulate gateway: wrap handle with rate-limit (e.g. 10 calls/min per key) + cost counter. 3) Add one Resource (e.g. "writing://recent") and one Prompt template. 4) Note for your Twin supervisor: how would critic use an MCP call through gateway? Complete → mark + share snippet (now includes secure MCP + gateway). Ties to L5 gate + L6 observability.</p>
<div class="callout tip"><div class="ct">4-min production quick</div>list+call is the protocol; security + gateway + resources/prompts is what makes it prod-ready for your full MCP-exposed multi-agent Twin.</div>`,
quiz:[{q:"Adding a gateway in front of your Twin MCP server primarily protects against what at L6 scale?",o:["Faster calls","Unbounded tool usage / cost from supervisor multi-agent or external MCP clients; per-token limits + auth + filtering contain blast radius","Simpler code"],a:1,e:"Gateways turn raw MCP exposure into governed capability — critical when supervisor routes many calls through your voice Twin tools."}]},
...
{id:"l5twin3",t:"🧪 Twin Stage 3: serve your twin (MCP + full multi-agent culmination)",min:14,lab:true,src:"LEH ch.9 §RAG Inference · ch.10–11",body:`
<p>The finale of the running build (spine: L2 function calling → L3 RAG+agents/multi-agent patterns → L5 MCP exposure + FTI deploy → here L5 supervisor multi-agent with Twin core → L6 scaling). You have your writing retrievable (Stage 1) and a model tuned to your voice (Stage 2). Stage 3 combines them into a deployed, monitored service — the <strong>inference pipeline</strong> of your [[fti|FTI]] system, live on the internet. This is your Twin, fully assembled — and now explicitly <strong>exposed as an MCP server</strong> and evolved into a <strong>full multi-agent system</strong> (supervisor + specialist agents using your voice Twin as a core worker). <strong>This is the explicit end-state of the course: a full MCP-exposed multi-agent system</strong> (MCP tools/list + tools/call + supervisor orchestration). Your deployed Twin is consumable by other agents via MCP. Labs/gate/quizzes here + sharing snippets all explicitly highlight the culmination.</p>
...
<h2>Step 4 — Expose as MCP server (the culmination)</h2>
<p>Wrap your deployed inference endpoint + your Twin tools (recall, ask_twin, list_writing, etc.) behind a full MCP server (JSON-RPC tools/list + tools/call + resources for your writing index + prompts for voice style). Any MCP client (Claude Desktop, Cursor, IDEs, other agents via OpenAI SDK/ADK/LangGraph adapters) can now discover (list) and invoke (call) your personal voice+memory+retrieval tools without reimplementation. Use FastMCP/@mcp.tool or SDK; support both stdio (local) and Streamable HTTP (remote deploy). Apply security: consent gates, arg validation, least-privilege. 2026: register in community MCP registry; present via code-execution-with-MCP (FS modules, sandbox exec for 90-98% token cuts, state via FS/skills). For SaaS reach: compose with Composio-managed MCP server. Front with gateway (OAuth, rate/cost limits, tracing). This is the explicit payoff of the agent spine: your Twin is now infrastructure.</p>
<h2>Step 5 — Multi-agent orchestration (full MCP-exposed supervisor Twin)</h2>
<p>Evolve the single Twin endpoint into a supervisor multi-agent system (required for L5 gate/Twin completion): a Supervisor (orchestrator) routes to (a) your voice-tuned Twin as Writer specialist, (b) a grounded Retriever specialist (your vector store), (c) a Critic that reviews for faithfulness/voice before final output. Use the supervisor/subagents (or handoffs/router/hierarchical) pattern you built + deepened in Level 3 (LangGraph subgraphs, OpenAI handoffs, ADK teams). Backed by your real deployed + MCP-exposed Twin core. Log every handoff/state transition with tracing. Add shared state + reducers + LangGraph Postgres/Redis checkpointers for persistence (durable long runs, resume, time-travel). For scale/tool volume: use code-mode MCP (Anthropic pattern) + gateways. Measure the multiplier (expect 1.5-3×); "start simple, measure" before full orchestration. Your Twin is now a production-grade full MCP-exposed multi-agent system (supervisor + voice writer + retriever + critic; tools listable/callable). This is the course culmination.</p>
<div class="callout"><div class="ct">What you built</div>A personal AI that knows your writing and speaks in your voice, designed as an FTI system, deployed behind your own API, exposed via MCP, orchestrated as a multi-agent system with your Twin at the core — traced, evaluated, and ready to be consumed by other agents. Built end to end. That is the LLM Engineer's Handbook's flagship + the full modern stack. You have shipped the complete thing.</div>`,
quiz:[
{q:"DIAGNOSTIC: Your Twin has RAG + function tools from L3. At Twin Stage 3, what specific production upgrade makes it the full MCP-exposed multi-agent system (the course culmination)?",o:["Only RAG deployment","Expose the tools via MCP (list+call server) and wrap the voice-tuned Twin in a supervisor that routes to writer (Twin) + retriever + critic specialists","Add a database only"],a:1,e:"Mental model: L5 is where private tools become pluggable MCP + orchestrated multi-agent with your fine-tuned Twin at core. Common mistake: thinking deployment alone = culmination. Check this question: this is the explicit full MCP-exposed multi-agent system; share snippets highlight it."},
...
project:{id:"l5gate",t:"Project Gate 5 — Deploy your RAG system as a real service (LLM Twin FULL MCP-EXPOSED MULTI-AGENT SYSTEM cap — EXPLICIT CULMINATION: MCP server exposed + supervisor multi-agent with Twin core REQUIRED)",
body:`
<p>Take your Level 3 project (now your full Twin) and give it the full production + sharing treatment. (Spine culmination: L2→L3→L5 MCP+FTI→L5 supervisor multi-agent Twin core; L6 will scale it on cost/latency/affinity/batching/persistence.) Deploy, then expose via MCP and wire a multi-agent supervisor around it. At the end, your Twin is not just live — it is an <strong>MCP-exposed multi-agent system</strong> (supervisor routes to your voice Twin + retriever + critic; tools exposed for any MCP client). This is a portfolio piece that proves the entire spine and culminates the agent build. Gate checklist, l5twin3 lab, and all sharing snippets make the full MCP multi-agent explicit.</p>
...
<h2>Part D — Culmination: full MCP-exposed multi-agent Twin (required for Twin completion + L5 gate pass; your explicit capstone)</h2>
<ul>
<li>Build a full-spec MCP server that exposes your Twin (JSON-RPC tools/list + tools/call discovery; include at least Tools + one Resource + one Prompt): tools like <code>ask_my_twin</code> (voice RAG), <code>recall_memories</code>, <code>list_my_writing</code>; register via FastMCP/etc; support stdio <strong>AND</strong> Streamable HTTP. Add security (consent, validate, least-priv). Consider code-execution MCP + Composio for scale. Verify any MCP client can list+call.</li>
<li>Evolve into supervisor multi-agent (2026 patterns, required): implement supervisor/subagents (with handoffs/shared state/reducers) routing to your Twin core as Writer + Retriever specialist + Critic. Use LangGraph (with checkpointing/persistence) or equiv; log handoffs + state. Verify supervisor produces traceable outputs using MCP-exposed Twin. This is the full MCP-exposed multi-agent system (supervisor + voice writer + retriever + critic).</li>
<li>Gate pass requires BOTH: "MCP server exposed" (list+call verified) + "supervisor multi-agent with Twin core" (handoffs logged, critic in path). Measure cost multiplier vs single Twin. Add persistence checkpoint for durable runs.</li></ul>
<h2>Stretch</h2>
<p>Add a nightly re-indexing job (orchestrated or simple scheduled script) so document changes flow to production automatically — completing your FTI architecture. Or publish your MCP server config for the cohort to consume.</p>`,
checklist:[
...
"My Twin is exposed as a FULL-SPEC MCP server (tools/list + tools/call verified + at least one resource/prompt + stdio or Streamable HTTP + basic security) — REQUIRED for Twin completion",
"My Twin runs inside a supervisor multi-agent system WITH TWIN CORE (supervisor routes to voice writer + retriever + critic; shared state/reducers, handoffs logged, checkpoint/persistence used, MCP tools consumed by agents) — explicit full MCP-exposed multi-agent culmination",
"Posted your full MCP+Multi-Agent Twin share snippet (MD/YAML with badges + tools + 'full MCP-exposed multi-agent system') via 🚀 — mandatory cohort ritual evidence"
]}
});
