...
videos:[
{title:"Horace He — Making Deep Learning Go Brrrr From First Principles (blog)",url:"https://horace.io/brrr_intro.html",why:"The definitive, readable explanation of compute-bound vs memory-bound vs overhead. This is the ops:byte bible — read it twice."},
{title:"3Blue1Brown — Essence of Linear Algebra (playlist)",url:"https://www.youtube.com/playlist?list=PLZHQObOWTQDPD3MizzM2xVFitgF8hE_ab",why:"Math support: if matrices and vectors feel shaky, this builds the intuition behind every FLOP your GPU computes. Optional but clarifying."}]},
...
<ul>
<li><strong>[[vllm|vLLM]]</strong> — the open-source default. [[paged attention|PagedAttention]], [[continuous batching]], prefix caching, quantized-model support, speculative decoding — and an OpenAI-compatible API server, so <em>the client code you've written all course works unchanged against your own GPU</em>.</li>
...
<li><strong>[[pipeline parallelism|Pipeline parallelism]]</strong> — layers 1–20 on GPU A, 21–40 on GPU B; tokens flow through like an assembly line. Simpler interconnect needs, but per-token latency doesn't improve. Mostly for fitting truly enormous models.</li>
<li><strong>[[expert parallelism|Expert parallelism]]</strong> — for [[moe|Mixture-of-Experts]] models: experts spread across GPUs; each token visits only its few routed experts. The throughput-efficient way to serve modern MoE frontier models.</li>
...
<li><strong>[[zero-downtime deploy|Zero-downtime deploys]]</strong> — roll out a new model/engine version with [[blue-green deployment|blue-green]] or rolling strategies so users never see an outage, and you can instantly roll back (the registry/versioning discipline from Level 5).</li>
